...
35. [x] continue from 32. when killing Claude code I want it to store the session metadata like model, mode so that when resume it, it can be resumed with the correct settings. Claude code has a hook on exit, we can use that hook to save its metadata and upon resuming we can use that metadata to restore the exact settings for the Claude session
36. [x] the web browser tab takes too long to load, is there anyway to preload the web engine first to make it faster. however, this must be a configurable.
37. [ ] the hot-key window share some settings style with other. I think these are global settings (theme, tab bar location). However, since hotkey windows is separate, I don't want it to share such things so it can have separate theme and tab bar appearance.
38. [x] when open the web browser tab, it has separate color theme and it override existing profile one. I tried to set it up so that it's consistent with the main profile I use but it stay the same (dark theme, with dark background for the tab bar). how to fix this?
39. [ ] when using web browser tab, hotkeys (like Cmd W) are captured by the web app. How can I configure it so that it can fall back to iTerm to close the tab ?
40. [ ] Sometimes all the tab and tab groups are not clickable. Then if I create a new tab (cmd T) then they all become clickable again.
41. [ ] After finishing editing a group name, I pressed enter to save it. At this point, I expect the focus will be the terminal. However, the focus is not changed to term, I have to left click in the terminal to shift the focus so I can start using it. Fix it so that after pressing enter (finishing editing group name), the focus automatically switch to the terminal of the selected session (or last selected session in the group whichever one is more straight forward)
