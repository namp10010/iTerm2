...
35. [ ] continue from 32. when killing Claude code I want it to store the session metadata like model, mode so that when resume it, it can be resumed with the correct settings. Claude code has a hook on exit, we can use that hook to save its metadata and upon resuming we can use that metadata to restore the exact settings for the Claude session
